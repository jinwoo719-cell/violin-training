...
#  버전이 어긋났는지 **가장 먼저** 봅니다
# ══════════════════════════════════════════════════════════════
#  파일을 하나씩 올리다 보면 app.py 만 새것이고 music.py 는 옛것인 일이
#  생깁니다. 그러면 AttributeError 가 나는데 스트림릿 클라우드는 원인을
#  가려 버려서 무엇이 문제인지 알 수가 없습니다.
#
#  **이 검사는 어떤 모듈 속성보다 먼저 와야 합니다.**
#  (한 번 아래쪽에 두었다가, 위에서 먼저 터져서 소용이 없었습니다)
#| 갈래  필요한 것이 다 있나 ? 계속한다 : 무엇이 옛 버전인지 알리고 멈춘다
_NEED = {
    "screens":   ["guide", "guide_component", "store", "paginate"],
    "music":     ["POSITIONS", "DIRECTIONS", "build_notes", "bend"],
    "sheet":     ["build", "list_models", "PREFERRED"],
    "staff":     ["line", "note_x"],
    "analyze":   ["analyze", "demo_wav"],
    "report":    ["build", "overall_score"],
}
_mods = {"screens": screens, "music": music, "sheet": sheet,
         "staff": staff, "analyze": analyze, "report": report}
_old = sorted({f"{m}.py" for m, fns in _NEED.items()
               for f in fns if not hasattr(_mods[m], f)})
if _old:
    st.error(f"### ⚠ {' · '.join(_old)} 가 옛 버전입니다\n\n"
